...
function compileHTML() {
   ...
   for (let dependency in dependencies) {
      if (dependencies[dependency].css_link !== undefined) {
         css_links +=
         '<link href="' +
         dependencies[dependency].css_link +
         '" rel="stylesheet">';
      }
      if (dependencies[dependency].js_link !== undefined) {
         js_links +=
         '<script src="' + dependencies[dependency].js_link + '"></script>';
      }
   }
...
function compileSCSS() {
   return gulp
      ...
      .pipe(
         inject.prepend(
         "/**" + os.EOL + credits.join(os.EOL) + os.EOL + "*/" + os.EOL + os.EOL
         )
      )
...
function compileJS() {
   ...
   .pipe(
      inject.prepend(
      "/**" + os.EOL + credits.join(os.EOL) + os.EOL + "*/" + os.EOL
      )
   )
...
function minifyJS() {
   return gulp
   .src("./src/assets/js/main.js", { allowEmpty: true })
  ...
  var stream = mergeStream();
   ...
   for (let dependency in dependencies) {
      if (dependencies[dependency].src) {
         stream.add(
         gulp
            .src(dependencies[dependency].src)
            .pipe(gulp.dest(distDir + dependencies[dependency].dest))
         );
      } else if (dependencies[dependency].srcs) {
         for (let multidependency in dependencies[dependency].srcs) {
         stream.add(
            gulp
               .src(dependencies[dependency].srcs[multidependency].src)
               .pipe(
               gulp.dest(
                  distDir + dependencies[dependency].srcs[multidependency].dest
               )
               )
         );
         }
